...
const Navbar = () => {
    const { usercontacts,setSearchOutputSelected,outputArray, setOutputArray} = useContext(userData)
    ...
    const filter = (e)=>{
       ...
       const output = usercontacts.filter((element)=>{
        ...
        outputArray.forEach(element => {
            setSearchOutput([...searchoutput,{lable:element.Email,value:element.Email}])
        });
    ...
    let  arr = [{lable:"abc",value:"abc"},{lable:"efd",value:"efd"}]
    return (
        <>
            <nav class="navbar navbar-light bg-light justify-content-between" className='navbar'>
                ...
                <AutoComplete style={{ width: "300px", marginLeft: "auto" }}  placeholder="search" 
